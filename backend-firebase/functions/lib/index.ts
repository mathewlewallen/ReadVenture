...
exports.updateParentSettings =
  exports.getReadingAnalytics =
  exports.getParentChildren =
  exports.updateUserSettings =
  exports.trackReadingProgress =
  exports.getChildrenAnalytics =
  exports.analyzeText =
    void 0;
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      pythonProcess.on('error', error => {
        ...
        reject(
          new functions.https.HttpsError(
            ...
            { type: AnalysisErrorType.PYTHON, details: error },
          ),
        );
      });
      pythonProcess.on('close', async code => {
        if (code !== 0) {
          console.error(
            `Analysis process failed with code ${code}:`,
            errorOutput,
          );
          reject(
            new functions.https.HttpsError(
              ...
              { type: AnalysisErrorType.PYTHON, code, error: errorOutput },
            ),
          );
          return;
        }
        try {
          ...
          try {
            await db.runTransaction(async transaction => {
              ...
              if (!userDoc.exists) {
                throw new functions.https.HttpsError(
                  'not-found',
                  'User not found',
                  { type: AnalysisErrorType.DATABASE },
                );
              }
              ...
              transaction.update(userRef, {
                'progress.totalWordsRead':
                  (userData.progress?.totalWordsRead || 0) + wordCount,
              });
            });
            resolve({
            ...
            reject(
              new functions.https.HttpsError(
                ...
                { type: AnalysisErrorType.DATABASE, details: dbError },
              ),
            );
          }
        } catch (parseError) {
          ...
          reject(
            new functions.https.HttpsError(
              ...
              { type: AnalysisErrorType.PARSING, details: parseError },
            ),
          );
        }
      });
    ...
    throw new functions.https.HttpsError(
      ...
      { type: AnalysisErrorType.VALIDATION, details: error },
    );
  }
});
  ...
  try {
    ...
    const analyticsPromises = childrenQuery.docs.map(async childDoc => {
      ...
      return {
        ...
        progress: {
          totalWordsRead: progress.reduce(
            (sum, p) => sum + (p.wordCount || 0),
            0,
          ),
          storiesCompleted: progress.length,
          averageAccuracy:
            progress.reduce((sum, p) => sum + (p.accuracy || 0), 0) /
            progress.length,
    ...
    await db.runTransaction(async transaction => {
      ...
      transaction.update(userRef, {
        'stats.totalWordsRead':
          (userData?.stats?.totalWordsRead || 0) + wordCount,
...
exports.getReadingAnalytics = functions.https.onCall(async (data, context) => {
  try {
    ...
    analytics.averageAccuracy =
      analytics.readingSessions.reduce(
        (sum, session) => sum + (session.accuracy || 0),
        0,
      ) / analytics.readingSessions.length;
    return analytics;
  } catch (error) {
    throw new functions.https.HttpsError(
      'internal',
...
exports.updateParentSettings = functions.https.onCall(async (data, context) => {
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      'Failed to update parent settings',
      error,
    );
  }
});
