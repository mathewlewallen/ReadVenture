...
filename = 'adaptive_algorithm/models/svm_model.sav'  # Update with your model file name
loaded_model = pickle.load(open(filename, 'rb'))

def predict_reading_level(features):
    """Predicts the reading level using the loaded model."""
    feature_vector = vectorizer.transform([features])
    predicted_level = loaded_model.predict(feature_vector)[0]  # Use the loaded model
    return predicted_level

# ... (Your existing code) ... 

def simplify_text(text, adjustment_level):  # Add adjustment_level as a parameter
    """Simplifies the text based on the adjustment level."""
    doc = nlp(text)
    simplified_text = []

    for sent in doc.sents:
        # 1. Shorten long sentences
        if len(sent) > 15 and adjustment_level >= 2:  # Adjust threshold based on level
            simplified_text.extend([str(sent)[:15], str(sent)[15:]])
        else:
            simplified_text.append(str(sent))

    simplified_text = " ".join(simplified_text)

    # 2. Replace difficult words with simpler synonyms
    if adjustment_level >= 1:  # Apply only if adjustment level is high enough
...
def analyze_text(text, story_id):
    ...
    # Example logic for dynamic adjustment (you'll need to refine this)
    child_reading_level = get_child_reading_level(story_id)  # Get the child's reading level (you'll need to implement this)
    
    adjustment_level = 0  
    if predicted_level > child_reading_level + 1:
        adjustment_level = 2  # High adjustment
    elif predicted_level > child_reading_level:
        adjustment_level = 1  # Moderate adjustment

    adjusted_text = simplify_text(text, adjustment_level)  # Pass adjustment level to simplify_text
