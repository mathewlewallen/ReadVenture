...
class ApiClient {
  ...
  private static addTimestampToRequest(config: AxiosRequestConfig): AxiosRequestConfig {
  ...
  private static handleResponseError(error: AxiosError): Promise<any> {
    ...
    if (error.message === 'Network Error' && originalRequest._retry !== this.retryCount) {
